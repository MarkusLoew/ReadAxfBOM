...
ReadAxfBOM <- function(data) {
	...
	# individual sections of the file start with "[someword]" and end with "[$]"
	...
	# the notice, header, and data blocks
	...
	the.data <- utils::read.csv(data, skip = data.location[1], nrows = data.rows)
	...
	# midnight is converted to a different number format than other times of the day during import
	the.data$local_date_time_full <- sprintf("%.0f", the.data$local_date_time_full)

	# convert to POSIX time
	the.data$Timestamp <- as.POSIXct(the.data$local_date_time_full, format = "%Y%m%d%H%M%S")
        # get rid of variable "sort_order" as it will prevent to identify duplicate samples
	...
	# identify special weather events which are outside of the 30min measurement frequency
